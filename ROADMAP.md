...
- Ensure web merge logic and MCP writes do not conflict.
  ...
  - 2026-06-10: live `capture_note` dry-run plus disposable `get_tasks` / `add_task` / `add_subtask` / `delete_task` smoke passed through authenticated MCP clients, and MCP/web refresh consistency is now fully closed. Final Claude Code / Fable 5 plus authenticated Chrome proof at repo HEAD `0d8903a` and production deployment `dpl_8md2CxUakCAZd68Ajp3NJVxsCgxs`: baseline `active=7`, fingerprint `33c25b0b`, outbox all zero; MCP created disposable task `5b4dc666-4793-45cd-8641-0ebc92398450` plus subtask `29f9b30d-c7a4-4953-9e2d-9df8b6e0502b`; premature bootstrap-loading packet was correctly rejected by `check:qa-packet`; hard-refresh post-write packet showed `active=8`, fingerprint `c12bc40c`, exact QA task/subtask; cleanup returned final packet to `active=7`, fingerprint `33c25b0b`; baseline-to-final `--expectStable --expectOutboxEmpty` passed. No non-QA task was touched and no capture was written.
  - 2026-06-10: Claude Code / Fable 5 independently verified the remote MCP `capture_note` dry-run tool call against `https://mcp.valquilty.com/mcp`: read-only `get_tasks` used the canonical tasks subcollection (`count=61`, `score=506` before manual cleanup; `count=60`, `score=521` after manual cleanup), and `capture_note` returned `origin.channel=mcp`, `origin.via=captures_api`, `origin.source=mcp:live-smoke`, `activeTasksSource=none`, `activeTasksCount=0`, and `captureId=dryrun-1781101401729` without mutations. No MCP-consistency follow-up remains open.
