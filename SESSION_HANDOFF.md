...
- As of 2026-06-01, Progress Decision Safety has `Copy QA baseline`, which copies auth mode, user id, active/today/at-risk/action counts, outbox counts, mission, delivery, Engine decision, report, and event counts. If it says `guest-or-local`, do not treat the run as authenticated live QA.
