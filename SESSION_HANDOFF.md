...
- As of 2026-06-06, `tests/telegram-callback-cancel.test.mjs` covers the second half of that boundary: `cancel:<taskId>` resolves to no planner command/no mutation, while `confirm_kill:<taskId>` still resolves to `KILL_TASK`. Real Telegram-client `Cemetery -> Cancel` evidence remains pending.
