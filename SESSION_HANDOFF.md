...
As of 2026-06-10, MCP/web refresh consistency smoke is **fully closed**. The current remaining items are **not known breakages**. They are deliberately unfinished live-verification steps that require the user, credentials, or explicit permission to perform a real write:
...
- As of 2026-06-10, `tests/planner-telegram-readonly-actions.test.mjs` also covers the Telegram `schedule_task` executor with mocked Google Calendar functions: disconnected users get the connect CTA, connected users missing date/time get guidance, and a connected valid route calls `createCalendarEvent` with the expected event payload. No live Google API call is made.
- As of 2026-06-10, `docs/google-calendar-live-smoke-checklist.md` is the canonical user-assisted Calendar live-smoke runbook. It keeps OAuth-only proof separate from optional real event creation and lists the evidence to capture without printing tokens.
...
- As of 2026-06-10, `npm run check:planner-integrity -- --backup <backup.json>` checks a local backup for planner semantic risks without network, Firestore reads, or Firestore writes. Current invariants cover dead tasks missing `deadAt`, invalid `deadlineAt` values such as `0020-02-07`, stale not-your-move blocks, Angel pins on non-active tasks, overdue active vital/Angel tasks, and QA/test/smoke task markers outside completed status.
- As of 2026-06-10, deadline write validation rejects or ignores dates outside `2020..2100`. API/client contracts, command-service task creation/deadline updates, and the standalone MCP add/set-deadline tools reject `0020-02-07`; capture/Telegram heuristic extraction drops invalid dates instead of writing them. This guard is deployed to production web/API (`dpl_8md2CxUakCAZd68Ajp3NJVxsCgxs`) and the Hetzner MCP mirror; MCP deploy backups are `index.js.backup-deploy-20260610T151939` and `capture-client.js.backup-deploy-20260610T151939`.
...
- As of 2026-06-06, `/api/captures` no longer marks every capture API write as web-origin. `source=mcp`, `source=mcp:*`, and Claude-MCP-like source strings produce `origin.channel: "mcp"`; `source=api:*` produces `origin.channel: "api"`. The dry-run handler returns that origin and `tests/captures-origin-contract.test.mjs` covers it without Firestore writes. Live Hetzner MCP dry-run capture wiring later passed through real clients; non-dry-run capture remains intentionally untested.
...
- As of 2026-06-10, the full authenticated MCP/web refresh smoke is closed. A live MCP client ran `capture_note`, `get_tasks`, `add_task`, `add_subtask`, and `delete_task` on a disposable `QA MCP smoke — delete after test` task only. Earlier web QA packets proved `972e7261 -> c6faf840` after the MCP write, then hard refresh kept `c6faf840` with the QA task/subtask still latest. Final Claude Code / Fable 5 plus authenticated Chrome proof at repo HEAD `0d8903a` and deployment `dpl_8md2CxUakCAZd68Ajp3NJVxsCgxs` proved exact restoration: baseline packet `2026-06-10T20:06:00.204Z` had `active=7`, `taskDataFingerprint=33c25b0b`, outbox all zero, mission `Выставить свитер Stone Island на продажу`; MCP created QA task `5b4dc666-4793-45cd-8641-0ebc92398450` and subtask `29f9b30d-c7a4-4953-9e2d-9df8b6e0502b`; a premature bootstrap-loading packet was rejected by `check:qa-packet` with `live_qa_not_ready`; post-refresh packet had `active=8`, fingerprint `c12bc40c`, exact QA task/subtask; cleanup returned final packet to `active=7`, fingerprint `33c25b0b`; baseline-to-final `--expectStable --expectOutboxEmpty` passed.
- As of 2026-06-10, Claude Code / Fable 5 independently closed the pending authenticated MCP `capture_note` dry-run tool-call smoke against `https://mcp.valquilty.com/mcp`. Read-only `get_tasks` returned `ok: true`, `documentExists: true`, canonical subcollection, `count: 61`, `score: 506` before manual user cleanup and `count: 60`, `score: 521` after manual cleanup. The `capture_note` request text was `MCP live-smoke dry run: verify capture tool origin only`, with `dry_run: true` and `source_label: live-smoke`; result metadata was `captureApi.status: 200`, `response.dryRun: true`, `origin.channel: mcp`, `origin.via: captures_api`, `origin.source: mcp:live-smoke`, `activeTasksSource: none`, `activeTasksCount: 0`, `captureId: dryrun-1781101401729`, and `idempotencyKeyPresent: false`. No capture was written. No MCP/web consistency follow-up remains open.
...
- As of 2026-06-10, the same fallback path also covers Telegram-style panic button/free-text variants with emoji, `SOS`, and curly apostrophes (`🆘 I’m stuck`, `SOS I’m stuck`, `sos`) as plain panic instead of `PANIC_TASK` with the button text as `task_ref`; quoted task-specific variants still become `PANIC_TASK`. Deploy/live Telegram free-text smoke is still required after this code reaches production.
- As of 2026-06-10, `tests/telegram-intent-fallback.test.mjs` also covers the `planner-agent-router` output for those OpenRouter-down panic strings, so `🆘 I’m stuck` must reach the executor as `panic` and not as `add_task` with task-memory enrichment.
