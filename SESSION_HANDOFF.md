...
- As of 2026-06-03, Google login uses Firebase redirect sign-in instead of popup sign-in, with `browserLocalPersistence` forced before redirect/result handling. The Codex in-app browser could navigate the popup flow into `telegrammadhd.firebaseapp.com/__/auth/handler` without an opener and leave the user on a blank callback page; the same embedded browser also reports no IndexedDB, so redirect auth must not depend on IndexedDB-backed persistence.
