...
- As of 2026-06-07, `npm run backup:planner -- --userId U2geUdbvWyVRNLWnSZBnftOMSU22 --dry-run` passed, confirming the default backup collection scope and ignored `backups/` output path without Firestore access. First real export remains pending.
...
- As of 2026-06-08, `npm run deploy:mcp-server` is the controlled deploy helper for the MCP mirror. It is dry-run by default; `-- --apply` uploads only `services/mcp-server/src/index.js`, checks syntax locally and remotely, backs up live `index.js`, replaces it, restarts PM2 `adhd-mcp`, and verifies `/healthz` plus the `/mcp` Bearer auth boundary. It does not copy secrets or Firestore data.
...
- As of 2026-06-07, that fresh post-OAuth Codex thread completed a controlled live MCP write smoke with no non-QA task touched: baseline `get_tasks` was `count: 61`, `score: 511`; `add_task` created `QA MCP smoke — delete after test` (`d56aa293-4768-4c4b-bb30-d186bf9bdfe0`); `add_subtask` added `QA MCP subtask write — delete after test` (`50fd06a6-d1e9-4656-b876-e5da1330c729`); verification `get_tasks` saw `count: 62` with the exact subtask; `delete_task` cleaned up the QA task; final `get_tasks` returned `count: 61`, `score: 511`, with the QA task absent. Web refresh/QA-packet proof remains separate because the disposable task was cleaned up immediately after MCP verification.
...
- As of 2026-06-07, `tests/captures-origin-contract.test.mjs` also covers the non-dry-run MCP-origin capture path with injected dependencies: `source=mcp:...` is passed to append-only capture storage with MCP origin metadata, `processCapture` runs on the stored capture, and no task mutation tool is involved. This is repo/API contract coverage; a live Hetzner MCP capture tool is still pending.
...
- As of 2026-06-06, `docs/mcp-live-smoke-checklist.md` is the canonical real-client procedure for Hetzner MCP verification. It separates read-only task listing, one disposable add-subtask mutation, web refresh/bootstrap evidence, cleanup, and optional `source=mcp:live-smoke` capture dry-run. The authenticated task read/write/cleanup piece has now passed in a fresh post-OAuth Codex thread, and the deployed capture API dry-run origin smoke passed on 2026-06-08. Web refresh/bootstrap evidence and a dedicated live Hetzner MCP capture tool remain pending.
