...
- As of 2026-06-06, `docs/mcp-live-smoke-checklist.md` is the canonical real-client procedure for Hetzner MCP verification. It separates read-only task listing, one disposable add-subtask mutation, web refresh/bootstrap evidence, cleanup, and optional `source=mcp:live-smoke` capture dry-run. The authenticated task read/write/cleanup piece has now passed in a fresh post-OAuth Codex thread; web refresh/bootstrap evidence and optional capture-origin smoke remain pending.
