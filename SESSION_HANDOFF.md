...
- As of 2026-06-06, Telegram fallback intent parsing no longer depends on JavaScript ASCII word boundaries for core Cyrillic actions. If OpenRouter is unavailable, plain-text commands for Done, revive from Heaven/Cemetery, Cemetery, Today pin/unpin, and critical on/off should route deterministically instead of falling through as accidental new-task adds. Live Telegram smoke is still required to prove the deployed bot path with real messages.
