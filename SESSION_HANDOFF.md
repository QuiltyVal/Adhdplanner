...
- As of 2026-06-03, Google login uses Firebase redirect sign-in instead of popup sign-in. The Codex in-app browser could navigate the popup flow into `telegrammadhd.firebaseapp.com/__/auth/handler` without an opener and leave the user on a blank callback page; redirect sign-in is the intended path for that embedded-browser access flow.
