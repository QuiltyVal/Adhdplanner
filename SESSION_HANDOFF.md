...
- As of 2026-06-05, task keyboards include a two-step `🪦 Cemetery` button. The first tap only sends the existing kill confirmation prompt (`Yes, Cemetery` / `Make smaller` / `Cancel` / `Open planner`); the task is moved to Cemetery only after `confirm_kill`. This needs production deploy and live Telegram smoke before closing the broader kill/revive roadmap item.
