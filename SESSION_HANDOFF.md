...
- As of 2026-06-02, Angel Lab shows a post-add next-state strip when some draft cards were handled and others remain. It confirms added cards are already in the planner, shows the remaining count and next draft card, and keeps `Done — back to planner` near the top.
- As of 2026-06-02, that post-add Angel Lab state suppresses the older bottom `Done — back to planner` action, so partial draft sessions have one clear return action instead of two duplicated exits.
