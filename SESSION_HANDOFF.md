...
- As of 2026-06-03, Google login uses Firebase redirect sign-in instead of popup sign-in, with `browserLocalPersistence` forced before redirect/result handling. The Codex in-app browser could navigate the popup flow into `telegrammadhd.firebaseapp.com/__/auth/handler` without an opener and leave the user on a blank callback page; the same embedded browser reports no IndexedDB and no Web Storage, so live auth through that browser is not viable. Login now detects missing Web Storage and tells the user to use Chrome/Safari plus QA packet instead of bouncing silently.
