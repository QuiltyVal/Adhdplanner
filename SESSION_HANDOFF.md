...
- As of 2026-06-03, the first authenticated live Angel Lab verification pass created exactly one QA task (`active: 8 -> 9`, `actionsToday: 0 -> 1`) and a post-run packet still showed `liveQaReady: yes`, clean outbox counts, and a fresh engine tick. The QA task still needs normal UI cleanup after recording the pass.
