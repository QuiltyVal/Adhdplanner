...
- As of 2026-06-02, Progress Decision Safety also has `Copy QA packet`, which combines the baseline and Decision Trace into one pasteable packet for QA notes or an agent handoff. Treat it as the primary copy action; use the separate baseline/trace buttons only when a narrower diagnostic is needed.
