...
- As of 2026-06-02, Angel Lab has a post-add next-state strip for partial draft sessions: after one card is handled and cards remain, the screen confirms the handled card is already in the planner, shows the remaining count and next draft card, and exposes `Back to planner — keep draft` near the top.
