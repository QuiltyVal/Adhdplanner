...
- As of 2026-06-08, `npm run deploy:mcp-server` provides a dry-run-first controlled Hetzner sync path for the mirrored MCP source. Apply mode performs local/server syntax checks, remote backup, source replacement, PM2 restart, and `/healthz` plus `/mcp` auth-boundary postchecks.
